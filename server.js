...
// Connect to MongoDB
mongoose.connect(
    'mongodb+srv://akshey54321:l9bJF0W0Q2T1mA35@cluster0.xvsl6.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0',
    {
...
const generateUniqueStudentID = async () => {
    ...
    do {
        studentID = Math.floor(
            Math.random() * (50000000 - 40000000) + 40000000
        );
...
app.post('/Signup', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({
            success: true,
            message: 'User signed up and saved to the database!',
        });
    } catch (error) {
...
app.post('/teams/:teamId/ratings', verifyToken, async (req, res) => {
    ...
    try {
        ...
        const isRaterInTeam = team.students.some((student) =>
            student._id.equals(req.user.id)
        );
        const isRateeInTeam = team.students.some((student) =>
            student._id.equals(rateeId)
        );

        if (!isRaterInTeam || !isRateeInTeam) {
            return res.status(403).json({
                message:
                    'Both the rater and ratee must be part of the same team.',
            });
        }
...
app.get('/teams/:teamId/ratings', verifyToken, async (req, res) => {
    try {
        ...
        const summary = team.students.map((student) => {
            const studentRatings = ratings.filter((rating) =>
                rating.ratee.equals(student._id)
            );
            const peersWhoResponded = studentRatings.length;

            if (peersWhoResponded === 0) {
                return { ...student._doc, average: 0, peersWhoResponded: 0 };
            }

            const cooperation =
                studentRatings.reduce(
                    (sum, r) => sum + (r.cooperation || 0),
                    0
                ) / peersWhoResponded || 0;
            const conceptualContribution =
                studentRatings.reduce(
                    (sum, r) => sum + (r.conceptualContribution || 0),
                    0
                ) / peersWhoResponded || 0;
            const practicalContribution =
                studentRatings.reduce(
                    (sum, r) => sum + (r.practicalContribution || 0),
                    0
                ) / peersWhoResponded || 0;
            const workEthic =
                studentRatings.reduce((sum, r) => sum + (r.workEthic || 0), 0) /
                    peersWhoResponded || 0;
            const average =
                (cooperation +
                    conceptualContribution +
...
app.get('/teams/:teamId/detailed-results', verifyToken, async (req, res) => {
    try {
        ...
        const ratings = await peerRatingModel
            .find({ team: teamId })
            .populate('rater', 'firstName lastName studentID');
...
app.get('/profile', verifyToken, async (req, res) => {
    try {
        const user =
            (await studentModel.findById(req.user.id).select('-password')) ||
...
app.put('/profile', verifyToken, async (req, res) => {
    ...
    try {
        const user =
            (await studentModel.findById(req.user.id)) ||
...
app.get('/student/meetings', verifyToken, async (req, res) => {
    try {
                ...
                .filter(
                    (booking) =>
                        ...
                        booking.attendees?.some((attendee) =>
                            attendee._id.toString() === studentId
                        )
                )
                .map((booking) => ({
                    _id: booking._id,
                    meetingName: booking.meetingName || 'N/A',
                    ...
                    admin: booking.student
                        ? `${booking.student.firstName || 'N/A'} ${booking.student.lastName || 'N/A'}`
                        : 'N/A',
                }))
        );
        ...
        console.error('Error fetching meetings:', error); // Log the exact error
        res.status(500).json({ message: 'Internal Server Error' });
    }
});
